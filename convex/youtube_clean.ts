...
export const transcribeVideo = action({
  ...
  handler: async (ctx, { userId, youtubeUrl, title }) => {
    ...
    try {
      ...
      transcriptId = await ctx.runMutation(api.youtube.createTranscript, {
        userId,
        youtubeUrl,
        status: "processing",
        originalScript: "",
      ...
      if (!videoId) {
        await ctx.runMutation(api.youtube.updateTranscript, {
          transcriptId,
          status: "error",
...
export const analyzeStyle = action({
  ...
  handler: async (ctx, { transcriptId, userId }) => {
    ...
    try {
      // Get the transcript
      const transcript = await ctx.runQuery(api.youtube.getTranscript, { transcriptId });
      if (!transcript) {
      ...
      const styleAnalysisId = await ctx.runMutation(api.youtube.createStyleAnalysis, {
        transcriptId,
        userId,
...
export const generateScript = action({
  ...
  handler: async (ctx, { userId, styleAnalysisId, inputTitle, inputDescription, scriptLength = "medium" }) => {
    ...
    try {
      ...
      const styleAnalysis = await ctx.runQuery(api.youtube.getStyleAnalysis, { styleAnalysisId });
      if (!styleAnalysis) {
        throw new Error("Style analysis not found");
      }

      // Get the original transcript for additional context
      const transcript = await ctx.runQuery(api.youtube.getTranscript, { 
        transcriptId: styleAnalysis.transcriptId 
      });

      // Create initial generated script record
      generatedScriptId = await ctx.runMutation(api.youtube.createGeneratedScript, {
        userId,
        transcriptId: styleAnalysis.transcriptId,
        styleAnalysisId,
        type: "generated",
      ...
      await ctx.runMutation(api.youtube.updateGeneratedScript, {
        generatedScriptId,
